        ...
        if categories_option == "Import from file":
            ...
            if cat_mood_file is not None:
                try:
                    ...
                    custom_categories, custom_moods = import_from_file(temp_path)
                    
                    # Clean up
                    os.unlink(temp_path)
                    
                    st.success(f"Imported {len(custom_categories)} categories and {len(custom_moods)} moods.")
            ...
            if categories_input:
                custom_categories = [c.strip() for c in categories_input.strip().split("\n") if c.strip()]
            
            if moods_input:
                custom_moods = [m.strip() for m in moods_input.strip().split("\n") if m.strip()]
            
            if custom_categories or custom_moods:
                st.success(f"Using {len(custom_categories or [])} custom categories and {len(custom_moods or [])} custom moods.")
        ...
        with st.spinner("Processing images..."):
            ...
            try:
                ...
                for uploaded_file in uploaded_files:
                    ...
                    image_paths.append({"path": temp_path, "name": uploaded_file.name})
                ...
                # Display the results
                st.success(f"Successfully processed {len(renamed_images)} images!")
...
def process_drive_images(export_format, custom_categories, custom_moods):
    ...
    if hasattr(st.session_state, 'drive'):
        ...
        try:
            ...
            folder_names = [f"{folder['name']} ({folder['id']})" for folder in folders]
            selected_folder = st.selectbox("Select a folder to process:", folder_names)
            
            # Get the folder ID from the selected folder
            folder_id = selected_folder.split("(")[-1].strip(")")
            
            # Output folder name
            output_folder_name = st.text_input("Output folder name:", "Processed Photos")
            
            if st.button("Process Images"):
                with st.spinner("Processing images from Google Drive..."):
                    try:
                        # Create output folder
                        output_folder_id = create_output_folder(drive, output_folder_name)
                        ...
                        st.success(f"Successfully processed {len(renamed_images)} images!")
                        st.info(f"Processed images and metadata saved to '{output_folder_name}' folder in Google Drive")
